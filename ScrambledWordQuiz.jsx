...
// Hna kanjibo l-icons mn 'lucide-react'
// F l-browser, ghadi yjiwhom mn l-script "lucide" li f index.html
const { Trophy, ArrowRight } = window.lucide || { Trophy: () => '🏆', ArrowRight: () => '➡️' };
...
export default function ScrambledWordQuiz() {
  ...
  const scrambleWord = (word) => {
    ...
    // Algorithm dyal Fisher-Yates shuffle
    ...
    // Kant2kdo an l-klma mkhrbqa mzyan
  ...
  const loadNewWord = () => {
    ...
    if (usedWords.length === wordBank.length) {
      wordsPool = []; // N3awdo mn lwl ila kmlna l-bank
    }
  ...
  const handleSubmit = () => {
    if (timer === 0) return; // Ma tjawbch ila sala lwaqt

    if (userGuess.toUpperCase() === currentWord.word) {
      setScore(score + 100); // Zid 100 noqta
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-blue-900 via-blue-800 to-blue-900 flex items-center justify-center p-4 text-white">
        <div className="text-center">
          ...
          <h2 className="text-2xl text-cyan-300 mb-12">Unscramble the word before the time runs out!</h2>
          <button
